// profile-scraper.js (copied from Nearby scraper)
(function(){
  ...
  async function scrapeProfile(){
    ...
    function getProfileId(){
      const selectors = ['header .sc-rcepan-2.jZxKik p', '.sc-rcepan-2.jZxKik p', 'h1 p', '.eeZNnP'];
    ...
    data.image_base64 = '';
    if(data.image){
      try{ data.image_base64 = await imageToBase64(data.image); }catch(e){ data.image_base64 = ''; }
    }
  ...
  function clearOverlays(){ try{ __romeo_overlays.forEach(n=>n.remove()); }catch(e){} __romeo_overlays = []; }
